...
Phases 0, 1 and 2 are complete (foundation; sign-in and shared data; Home, search, Settings). Phase 3 (create/edit/delete chores, links in descriptions, photos) is complete and committed. Phase 4 (chore detail, full-screen photos, status and Mark as Done, live comments, calendar) is code-complete and simulator-verified; its two-device and calendar checks are still open. Phase 5 (push notifications) is in progress: the native side (permission, device registration, tap routing, outbound events) is built and simulator-tested; the backend delivery over APNs and the device checks are next. See `docs/phase-N-validation.md` for what has actually been verified, not just implemented.
