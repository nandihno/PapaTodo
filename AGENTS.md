...
- **Launch arguments** (fixtures only): `-UITestFixtures`, `-UITestFailFirstLoad`, `-UITestDarkMode`, `-UITestSeedPhoto`, `-UITestStorageRefusesDeletes` (the fixture bucket refuses file deletes, like the live one today), `-UITestRemoteComment` (another client comments on the recycling chore about 1.5 s after the detail screen subscribes).
